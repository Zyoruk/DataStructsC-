...
public:
     ...
     * \return boolean if it was able to connect, the only way it wouldn't is if
     * the connection already exists. Or that one node doesn't exist.
     */
    ...
    bool operator == (Graph<K>* pGraphToCompare);
protected:
    /*!
     ...
     * \param closed
     */
    void searchforPaths (GraphNode<K> *pNodeA, GraphNode<K> *pNodeB, SimpleList<GraphNode<K> *>* pList,
                         Stack<GraphNode<K>* >* path, SimpleList<GraphNode<K>* >* closed);
};
template <typename K>
Graph<K>::Graph(){
...
template <typename K>
void Graph<K>::searchforPaths(GraphNode<K> *pNodeA, GraphNode<K> *pNodeB, SimpleList<GraphNode<K> *>* pList,
                              Stack<GraphNode<K>* >* path, SimpleList<GraphNode<K>* >* closed){
    path->push(pNodeA);
...
template <typename K>
bool Graph<K>::operator ==(Graph<K>* pGraphToCompare){
    if( this->_elements->getLenght() == pGraphToCompare->getElements()->getLenght()){
        if  (*this->_elements == *pGraphToCompare->getElements()){
            return true;
        }else{
            return false;
        }
